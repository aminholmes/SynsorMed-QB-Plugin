...
import com.quickblox.sample.groupchatwebrtc.fragments.CallRequestAlert;
...
public class CallActivity extends BaseLogginedUserActivity implements QBRTCClientSessionCallbacks, QBRTCSessionConnectionCallbacks, QBRTCSignalingCallback,
    ...
    private CallRequestAlert alert;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final String sessionID = getIntent().getStringExtra("callId");
        isProvider = getIntent().getBooleanExtra("isProvider",false);

        Log.d("AminLog","The session id is: " + sessionID);
        rtcClient = QBRTCClient.getInstance(getApplicationContext());
        currentSession = rtcClient.getSession(sessionID);
...
/*/
    ...
    public void cleanUP(){
        ...
        Log.d("AminLog","The call is complete, going to cleanup");

        currentSession = null;
        callStatus = "ENDED";

        //Tell JS side the call has ended
        //RtccAndroidPhonegap.endCallJS();
...
//                ringtonePlayer.stop();
    ...
    public void onSessionClosed(final QBRTCSession session) {
        ...
        Log.d("call_session", "onSessionClosed");
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
...
//            opponentsWithoutMe.remove(new Integer(myId));
            ...
            Log.d("tracks", "getCallerID " + session.getCallerID());
            Log.d("tracks", "myId " + myId);

//            QBUser myOpponent = new QBUser(session.getCallerID());
            QBUser myOpponent = new QBUser(session.getOpponents().get(0));
    ...
    private void showCallRequestAlert(){


        alert.listener = new CallRequestAlertInterface() {
            @Override
            public void hangUp() {
                hangUpCurrentSession();
                alert.dismiss();
